module.exports = async (req, res) => {
  try {
    ...
    console.error('処理中のエラー:', error.response ? error.response.data : error.message);
    return res.status(500).json({ message: 'サーバーエラー', error: error.response ? error.response.data : error.message });
  }
};
